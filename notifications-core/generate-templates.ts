...
function generateTemplates(event: string, channels: string[], langs: string[]) {
  ...
  for (const channel of channels) {
    for (const lang of langs) {
      ...
      const dir = path.join(templatesPath, ...event.split('/'), channel);
      ensureDir(dir);
...
if (!event || !channels.length || !langs.length) {
  console.error('Usage: npx ts-node notifications-core/generate-templates.ts --event mission/closed --channels whatsapp,sms,email --langs fr,en');
  process.exit(1);
}
